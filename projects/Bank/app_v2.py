...
users = {
    ...
    1236: {'username':'Pranav','email':'2211cs010503@mallareddyuniversity.ac.in','balance':50000},
    1237: {'username':'Rajiu','email':'2211cs010534@mallareddyuniversity.ac.in','balance':5000}
}
...
def register(username:str, email:str, password:str, balance:int)->str:
    with open("users.csv","r+",newline="") as file:
        ...
        writer_obj.writerow([account_no, username, email, balance])
        # add user account and password into accounts table
        with open("accounts.csv","a",newline="") as acc_file:
            acc_writer = csv.writer(acc_file)
            acc_writer.writerow([account_no, password])
...
if __name__ == "__main__":
    ...
    if choice == 1:
        ...
        password = input("Enter your password:")
        intial_deposit = int(input("Enter deposit amount: "))
        print(register(username=username, 
                       email=email, 
                       password=password, 
                       balance=intial_deposit))
    else:
        print("Select valid operation (1 or 2)")